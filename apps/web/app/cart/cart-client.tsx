...
export default function CartClient() {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://grocio-api-048w.onrender.com/api/v1"}/orders`, {
        method: "POST",
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === "login") {
        // Login
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://grocio-api-048w.onrender.com/api/v1"}/auth/login`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        // Signup
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://grocio-api-048w.onrender.com/api/v1"}/auth/register`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            email: formData.email,
